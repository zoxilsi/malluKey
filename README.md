...
MalluKey is a modern, fast, and secure web application built to help users practice and improve their Malayalam typing speed. It features a custom virtual Malayalam keyboard layout, real-time WPM (Words Per Minute) tracking, and a global Top 10 Leaderboard.
...
- **Zero Frontend API Keys:** Absolutely no sensitive database keys (like Supabase URLs or Anon/Service keys) are exposed to the client bundle, browser console, or frontend environment.
...
- **Secret Exclusion:** No `.env` or sensitive configurations are ever committed to the GitHub index history. These are strictly injected at deployment via the Vercel Dashboard.
...
3. Add the following keys manually (provided directly from your Supabase Dashboard):
   - `SUPABASE_URL`
